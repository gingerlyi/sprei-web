    ...
    <main className="min-h-screen bg-white font-sans text-slate-900 selection:bg-gray-200">
      ...
      <div className="max-w-7xl mx-auto px-6 pt-28 pb-12">
        <div className="flex gap-2 text-xs text-gray-500 mb-6 font-medium items-center">
          <Link href="/" className="hover:text-black">Beranda</Link> <span className="text-gray-300">&gt;</span> <span className="text-black">Katalog Sprei</span>
        </div>

        <div className="flex flex-col lg:flex-row gap-6">
          <aside className="w-full lg:w-64 flex-shrink-0 border border-gray-200 rounded-[20px] p-5 h-fit hidden md:block">
            <div className="flex justify-between items-center mb-2 border-b border-gray-100 pb-4"><h2 className="text-lg font-bold">Filters</h2></div>
            <div className="py-4 border-b border-gray-100">
              <h3 className="font-bold mb-4">Kategori</h3>
              <div className="flex flex-col gap-3 text-sm text-gray-500">
                <Link href={buildUrl({ newCategory: null })} className={`flex justify-between hover:text-black transition-colors ${!category ? 'text-black font-bold' : ''}`}><span>Semua Kategori</span></Link>
                <Link href={buildUrl({ newCategory: 'polos' })} className={`flex justify-between hover:text-black transition-colors ${category === 'polos' ? 'text-black font-bold' : ''}`}><span>Sprei Polos</span></Link>
                <Link href={buildUrl({ newCategory: 'motif' })} className={`flex justify-between hover:text-black transition-colors ${category === 'motif' ? 'text-black font-bold' : ''}`}><span>Sprei Motif</span></Link>
                <Link href={buildUrl({ newCategory: 'bedcover' })} className={`flex justify-between hover:text-black transition-colors ${category === 'bedcover' ? 'text-black font-bold' : ''}`}><span>Bedcover Set</span></Link>
              </div>
            </div>
            ...
            <Link href="/katalog" className="block text-center w-full bg-black text-white py-3.5 rounded-full text-sm font-medium hover:bg-gray-800 transition-colors">Reset Filter</Link>
          </aside>
          ...
          <div className="flex-1">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-4">
              ...
              <form method="GET" action="/katalog" className="relative w-full md:w-64">
                ...
                <input type="text" name="q" defaultValue={q} placeholder="Cari nama produk..." className="w-full bg-[#F0EEED] text-sm rounded-full py-2.5 pl-10 pr-4 focus:outline-none focus:ring-1 focus:ring-black" />
                <button type="submit" className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg></button>
              </form>
            </div>
            ...
            {products.length === 0 ? (
               <div className="p-16 bg-gray-50 rounded-2xl text-center border-2 border-dashed border-gray-200">
                  ...
                  return (
                    <Link href={`/product/${product.id}`} key={product.id} className="group cursor-pointer block">
                      <div className="bg-[#F0EEED] rounded-[16px] aspect-[4/5] overflow-hidden mb-3 relative">
                        <img src={product.imageUrls?.[0] || ""} alt={product.name} className="w-full h-full object-cover mix-blend-multiply transition-transform duration-500 group-hover:scale-110" />
                      </div>
                      <div>
                        <h3 className="font-bold text-base leading-tight mb-2 truncate text-black uppercase tracking-tight">{product.name}</h3>
                        {/* RATING DIHAPUS, HARGA NAIK KE ATAS */}
